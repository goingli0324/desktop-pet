...
跨平台（macOS／Windows）Electron 桌面寵物。內建貓、狗與三隻倉鼠（使用者自有 IP）；上傳一張圖可請 Gemini 生成 8 格動作表匯入。
...
node scripts/build-builtin.mjs [cat|dog|hamster-walk|hamster-stand|hamster-jump]   # 從 assets/sheets/<animal>.png 重產內建素材（原始表不進打包）
```
